...
-- Type your code below:
select count(temp.movie_id) as no_of_movies
from (select movie_id, count(genre) as c
from genre
group by movie_id
having c=1) as temp;
...
-- Type your code below:
select * 
from (select genre,count(movie_id) as movie_count, rank() over(order by count(movie_id) desc) as genre_rank
	from genre
	group by genre) as temp
where temp.genre='thriller';
...
+---------------+-------------------+ */
-- Type your code below:
select t.actor_name, t.movie_count
from (select name as actor_name, count(r.movie_id) as movie_count,
...
+-------------------+-------------------+---------------------+*/
-- Type your code below:
select t.production_company, t.vote_count, t.prod_comp_rank
from (
select production_company, sum(total_votes) as  vote_count,
	rank() over (order by sum(total_votes) desc ) as prod_comp_rank
	from movie as m
	inner join ratings as r  on r.movie_id=m.id
group by production_company) as t
limit 3;
...
select * 
from (select name as actress_name, sum(total_votes) as total_votes, count(m.id) as movie_count, 
...
select *,(case when t.avg_rating >=8 then 'Superhit movie'
        when t.avg_rating >=7 and t.avg_rating <8 then  'Hit movie'
        when t.avg_rating >=5.0 and t.avg_rating < 7 then  'One-time-watch movie'
        when t.avg_rating <5.0 then 'Flop movie' end) as  Class
from (select  title,avg_rating
	from genre as g 
...
select * 
from (select genre, year, title as movie_name, worlwide_gross_income,
			rank() over (partition by year order by worlwide_gross_income desc) as movie_rank
	from movie as m 
    inner join genre as g 
    on m.id= g.movie_id
	where genre in (select genre from  (select genre, count(movie_id) as number_of_movies
		from genre as g
		inner join movie as m
		on g.movie_id = m.id
		group by genre
...
from (select name_id as director_id, name as director_name, dir.movie_id, duration,
...
inner join ratings as rt on mov.id = rt.movie_id) as t
